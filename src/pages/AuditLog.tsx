  ...
  return (
    <div className="p-4 sm:p-6 max-w-5xl mx-auto">
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        <select value={tableFilter} onChange={(e) => setTableFilter(e.target.value)} className="text-sm rounded-lg border border-slate-300 px-2.5 py-1.5">
        ...
        <select value={actionFilter} onChange={(e) => setActionFilter(e.target.value)} className="text-sm rounded-lg border border-slate-300 px-2.5 py-1.5">
                ...
                <div key={l.id}>
                  ...
                  >
                    <div className="flex items-center gap-2 min-w-0">
                      <span className={`text-[10px] font-semibold uppercase px-2 py-0.5 rounded shrink-0 ${ACTION_COLOR[l.action] ?? 'bg-slate-100 text-slate-600'}`}>
                        {l.action}
                      </span>
                      <span className="text-sm font-medium text-slate-700 shrink-0">{l.table_name}</span>
                  ...
                  {expanded && (
                    <div className="px-4 pb-3 bg-slate-50 text-xs">
                      {l.action === 'update' ? (
                        changes.length === 0 ? (
                          <div className="text-slate-400 py-2">No field-level changes (only timestamps updated).</div>
                        ) : (
                            ...
                            <tbody>
                              {changes.map(([field, before, after]) => (
                                <tr key={field} className="border-t border-slate-200">
                                  <td className="py-1 pr-2 font-mono text-slate-600">{field}</td>
                        ...
                        <pre className="whitespace-pre-wrap text-slate-600 py-2">
