  ...
  return (
    <div className="p-4 sm:p-6 max-w-4xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-1">
        <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Warehouses</h2>
        {isAdmin && (
      ...
      {showAdd && (
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm p-4 mb-6 grid grid-cols-1 sm:grid-cols-2 gap-3 items-end">
          ...
          <div className="divide-y divide-slate-50 dark:divide-slate-700/60">
            {warehouses.map((w) => (
              <div key={w.id}>
                <div className="px-4 py-3 flex items-center justify-between gap-3 flex-wrap">
                  <button onClick={() => toggleExpand(w)} className="text-left flex-1">
                    <div className="font-medium text-sm text-slate-900 dark:text-slate-100 flex items-center gap-2">
                      {w.name}
                      {w.is_default && <span className="text-[10px] uppercase tracking-wide bg-indigo-50 text-indigo-600 rounded px-1.5 py-0.5 dark:bg-indigo-900/40 dark:text-indigo-300">Default</span>}
                    </div>
                    {w.address && <div className="text-xs text-slate-400 dark:text-slate-500">{w.address}</div>}
                  </button>
                  {isAdmin && (
                    <div className="flex items-center gap-3">
                      <label className="flex items-center gap-1 text-xs text-slate-500 dark:text-slate-400" title="Allocation priority: lower number is picked first when a SKU is spread across warehouses">
                        Priority
                        <input
                          type="number"
                          value={w.allocation_priority}
                          onChange={(e) => updatePriority(w, Number(e.target.value) || 0)}
                          className="w-14 text-sm rounded-lg border border-slate-300 px-1.5 py-0.5 dark:bg-slate-800 dark:border-slate-600 dark:text-slate-100"
                        />
                      </label>
                      {!w.is_default && (
                      ...
                      <button onClick={() => setDeleteTarget(w)} className="text-xs text-slate-400 dark:text-slate-500 hover:text-red-600">
                        Delete
                      </button>
                    </div>
                  )}
                ...
                {expandedId === w.id && (
                  <div className="px-4 pb-4 bg-slate-50 dark:bg-slate-700/40">
                    <div className="text-xs font-semibold uppercase text-slate-500 dark:text-slate-400 mb-2">Bins / racks</div>
                    <div className="flex flex-wrap gap-1.5 mb-2">
                      {(binsByWarehouse[w.id] ?? []).length === 0 ? (
                        <span className="text-xs text-slate-400 dark:text-slate-500">No bins yet.</span>
                      ) : (
                        (binsByWarehouse[w.id] ?? []).map((b) => (
                          <span key={b.id} className="inline-flex items-center gap-1 text-xs bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded px-2 py-1">
                            {b.code}
                            {isAdmin && (
                    ...
                    {isAdmin && (
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          className="text-sm rounded-lg border border-slate-300 px-2.5 py-1.5 w-32 dark:bg-slate-800 dark:border-slate-600 dark:text-slate-100"
                        />
